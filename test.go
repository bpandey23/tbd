...
func mergeSort(arr []int) []int {
	...
	fir := mergeSort(arr[:len(arr)/2])
	sec := mergeSort(arr[len(arr)/2:])
	return merge(fir, sec)
}

func merge(fir, sec []int) []int {
	merged := make([]int, 0)
	i, j := 0, 0
	for k := 0; k < (len(fir) + len(sec)); k++ {
		if i == len(fir) {
			merged = append(merged, sec[j])
			j++
		} else if j == len(sec) {
			merged = append(merged, fir[i])
			i++
		} else if fir[i] > sec[j] {
			merged = append(merged, sec[j])
			j++
